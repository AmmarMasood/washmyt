...
function NewWash(props: ICustomerDetailProps) {
  const [form] = Form.useForm();
  const { show, onClose, onConfirm, setLoading } = props;
  ...
  const getAllCustomers = async () => {
    setLoading(true);
    try {
      const response = await axiosApiInstance.get("/api/admin/customers/all");

      setCustomers(response.data.customers);
    } catch (error) {
      console.log(error);
      message.error("Unable to get customers");
    }
    setLoading(false);
  };
  ...
  useEffect(() => {
    if (selectedModel && options.length) {
      const mods = getModels(options);
      const filMods = mods.filter((f: any) =>
        f.model.includes(selectedModel.replace("model", ""))
      );
      setFilteredOptions(filMods);
    }
  }, [selectedModel]);
  useEffect(() => {
    getAllCustomers();
  ...
  const onFinishForm = async (values: any) => {
    ...
    setLoading(true);
    try {
      const res = await axiosApiInstance.post("/api/wash-request", values);
      message.success("Wash created successfully");
      form.resetFields();
      onConfirm();
    } catch (error) {
      console.log(error);
  ...
  return (
    ...
    >
      <div className="flex items-center">
        ...
        <h3 className="font-bold ml-4 text-lg">New Wash</h3>
      </div>
      <Form form={form} onFinish={onFinishForm}>
        <div className="mt-6">
          <div className={rowClassName}>
            <p className={keyClassName}>Customer</p>
            <div className={valueClassName}>
              ...
              >
                ...
                >
                  {customers.map((option: any, key: number) => (
          ...
          <div className={rowClassName}>
            <p className={keyClassName}>Model</p>
            <div className={valueClassName}>
              <Form.Item
                name="selectedModel"
                rules={[
              ...
              >
                <Select
                  ...
                  onChange={(e: any) => setSelectedModel(e)}
                >
                  {modelsData.map((option: any, key: number) => (
                    <Select.Option value={option.id} key={key}>
                      {option.name}
                    </Select.Option>
                  ))}
          ...
          <div className={rowClassName}>
            <p className={keyClassName}>Color</p>
            <div className={valueClassName}>
              <Form.Item
                name="color"
                rules={[
              ...
              >
                ...
                >
                  {colors.map((option: any, key: number) => (
                    <Select.Option value={option.id} key={key}>
                      {option.name}
                    </Select.Option>
                  ))}
          ...
          <div className={rowClassName}>
            <p className={keyClassName}>Add Snow Package (+$79)</p>
            <div className={valueClassName}>
              <Form.Item name="snowPackage">
                <Checkbox
                  className="bg-primary-red"
                  onChange={(e) => {
                    form.setFieldsValue({ snowPackage: e.target.checked });
                  }}
                />
              </Form.Item>
            </div>
