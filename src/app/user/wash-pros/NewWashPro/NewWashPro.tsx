...
function NewWashPro(props: ICustomerDetailProps) {
  const { show, onClose, onConfirm } = props;
  const [loading, setLoading] = React.useState(false);
  const [businessAddress, setBusinessAddress] = React.useState<any>(null);
  ...
  const onFinishForm = async (values: any) => {
    ...
    setLoading(true);
    try {
      await axiosApiInstance.post(`/api/admin/wash-pros/create`, values);
      message.success("Wash Pro create successfully");
      onConfirm();
    } catch (error) {
      console.log(error);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center">
        ...
        <h3 className="font-bold ml-4 text-lg">New Wash Pro</h3>
      </div>
      ...
      <Form form={form} onFinish={onFinishForm}>
        <div className="mt-6">
          ...
          <div className={rowClassName}>
            <p className={keyClassName}>Password</p>
            <div className={valueClassName}>
              <Form.Item
                name="password"
                rules={[
                  ...
                  {
                    min: 6,
                    message: "Aleast 6 characters",
                  },
                ]}
              >
                <Input type="password" width={350} />
              </Form.Item>
            </div>
          </div>
          <div className={rowClassName}>
